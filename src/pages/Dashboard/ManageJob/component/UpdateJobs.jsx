...
const UpdateJobs = () => {
  ...
  const onSubmit = async (updateData) => {
    const imageUrl = await uploadImage(updateData?.jobCircularImgUrl[0]);

    updateData.jobCircularImgUrl = imageUrl
      ? imageUrl
      : data?.jobCircularImgUrl;

    const result = await updateJob(data?._id, updateData);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <div className="form-control w-full  my-4">
            ...
            <label className="w-full">
              <input
                defaultValue={data?.deadline.split("T")[0]}
                type="date"
