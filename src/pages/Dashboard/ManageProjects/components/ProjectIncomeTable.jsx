...
import { getTotalIncome } from "../../../../utils/calculation";

const ProjectIncomeTable = ({ projectIncome }) => {
  const getSum = (list) => {
    let sum = 0;
    list?.map((income) => {
      sum += income?.amount;
        ...
        <table className="table border">
          ...
          <tbody>
            {projectIncome?.length ? (
              projectIncome?.map((income, i) => (
                <tr key={i} className="text-[12px] border shadow-lg">
                  <th>{i + 1}</th>
                  <td>{income?.date}</td>
                  <td>{income?.userId?.name || 'Unknown'}</td>
